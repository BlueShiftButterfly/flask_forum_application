...
def create_example_content(db: DatabaseBridge):
    ...
    if example_user is None:
        print("oh no")
        return
    pyforum = db.create_forum("python", "Python", "Forum for Python related discussion", example_user.db_id)
    qaforum = db.create_forum("questions", "Questions", "Forum for asking questions", example_user.db_id)
